...
export const NavMidItem = styled(Link)<ActiveProps>`
  position: relative;
  width: 100px;
  height: 100%;
  display: flex;
...
export const RoundedUserDiv = styled.div<ActiveProps>`
  background: ${({ active = false }) =>
    active ? "rgba(0,44,200,.2)" : "white"};
...
export const NewNotifications = styled.div<CountProps>`
  display: flex;
  align-items: center;
  justify-content: center;
  width: 16px;
...
export const Menu = styled.div`
  ...
  border-radius: 5px;
`;
export const NewFriendsNotifications = styled.div<CountProps>`
  display: flex;
  align-items: center;
  justify-content: center;
  width: 20px;
